...
public struct AgentInstructionSetFactory: Sendable {
    ...
    private static func instructionText(
        ...
        If a capability decision is provided, treat it as explicit guidance for capability, command, and target device unless it conflicts with the hydrated candidates or safety rules.
    ...
    private static func promptText(
        ...
        \(capabilityDecisionPrompt(input.capabilityDecision))
    ...
    private static func capabilityDecisionPrompt(_ decision: HomeCapabilityDecision?) -> String {
        guard let decision else {
            return ""
        }
        let alternatives = decision.alternatives.map {
            "- capability=\($0.capability), command=\($0.command ?? "none"), target=\($0.targetDeviceID ?? "none"), confidence=\($0.confidence), evidence=\($0.evidence.joined(separator: " | "))"
        }
        .joined(separator: "\n")
        return """

        Capability decision:
        - Selected capability: \(decision.selectedCapability ?? "none")
        - Selected command: \(decision.selectedCommand ?? "none")
        - Target device ID: \(decision.targetDeviceID ?? "none")
