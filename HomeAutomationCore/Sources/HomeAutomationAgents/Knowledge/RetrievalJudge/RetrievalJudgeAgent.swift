...
public struct RetrievalJudgeAgent: HomeAgent {
    ...
    public static func retryStrategy(for source: KnowledgeSource) -> AgentRetrievalStrategy {
        AgentRetrievalStrategy(
            name: "retrievalJudge.agentic.\(source.rawValue)",
            agentID: .retrievalJudge,
            purpose: "Retry weak RAG sources with a reformulated agentic query while preserving source-specific metadata filters.",
            source: source,
            requiredTags: source.isAutomationKnowledge ? ["operation": HomeAutomationOperationKind.automationCreation.rawValue] : [:],
            topK: 3,
            minScore: 0.01,
            ranking: .agentic,
            fallbackBehavior: .retryWithReformulation
        )
    }
    ...
    public func run(_ input: RetrievalJudgeInput, context: ResolutionContext) async throws -> KnowledgeRetrievalAgentOutput {
        ...
        for source in sources {
            let strategy = Self.retryStrategy(for: source)
            let query = strategy.makeQuery(
                rawText: input.text,
                semanticText: reformulated,
                keywordTerms: hints.deviceTypes + hints.rooms + hints.capabilities,
                requiredTagValues: Self.requiredTagValues(for: source, hints: hints),
                nluHints: hints,
                operation: source.isAutomationKnowledge ? .automationCreation : .executeDeviceCommand
            )
    ...
    private static func requiredTagValues(for source: KnowledgeSource, hints: NLURetrievalHints) -> [String: [String]] {
        switch source {
        case .capability, .bixbyCommand:
            return hints.deviceTypes.isEmpty ? [:] : ["relatedDeviceTypes": hints.deviceTypes]
        case .nlDataset:
            return hints.deviceTypes.isEmpty ? [:] : ["deviceType": hints.deviceTypes]
        case .device:
            return hints.deviceTypes.isEmpty ? [:] : ["deviceType": hints.deviceTypes]
...
private extension KnowledgeSource {
    var isAutomationKnowledge: Bool {
        switch self {
        case .automationPattern, .automationRuleExample, .automationConditionOperator, .smartThingsRuleSchema:
            return true
        case .capability, .nlDataset, .bixbyCommand, .device:
            return false
        }
    }
}
