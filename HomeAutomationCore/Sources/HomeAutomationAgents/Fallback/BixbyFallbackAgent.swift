...
public struct AgentBixbyDraftMatch: Sendable, Hashable {
...
public struct BixbyFallbackInput: Sendable {
    public let text: String
    public let devices: [HomeCandidateRecord]

    public init(text: String, devices: [HomeCandidateRecord]) {
        self.text = text
        self.devices = devices
    }
}

public struct AgentBixbyFallbackMapper: Sendable {
    public init() {}

    public func matches(text: String, devices: [HomeCandidateRecord]) -> [AgentBixbyDraftMatch] {
        var bestByKey: [String: AgentBixbyDraftMatch] = [:]
        ...
        for device in devices {
            for name in deviceNames(for: device) {
                ...
                for command in commands {
                    ...
                    let match = AgentBixbyDraftMatch(
                        device: device,
                        draft: draft,
    ...
    private func mappedCommand(
        ...
        text: String,
        device: HomeCandidateRecord
    ) -> AgentBixbyCommandMapping? {
        let action = command.action.agentCompactHomeTokenString
        let sourceCapability = command.capability.agentCompactHomeTokenString
        ...
        if isRead, let capability = statusCapability(for: sourceCapability, device: device) {
            return AgentBixbyCommandMapping(
                intent: .getStatus,
        ...
        if action.contains("open"), let capability = firstSupported(device, ["garageDoorControl", "doorControl", "windowShade", "valve"]) {
            return AgentBixbyCommandMapping(intent: .open, capability: capability, command: "open", parameters: [], score: 29)
        }
        if action.contains("close"), let capability = firstSupported(device, ["garageDoorControl", "doorControl", "windowShade", "valve"]) {
            return AgentBixbyCommandMapping(intent: .close, capability: capability, command: "close", parameters: [], score: 29)
        }
        if action == "unlock", device.capabilities.contains("lock") {
            return AgentBixbyCommandMapping(intent: .unlock, capability: "lock", command: "unlock", parameters: [], score: 31)
        }
        if action == "lock", device.capabilities.contains("lock") {
            return AgentBixbyCommandMapping(intent: .lock, capability: "lock", command: "lock", parameters: [], score: 31)
        }
        if ["play", "pause", "stop", "fastforward", "rewind"].contains(action),
           device.capabilities.contains("mediaPlayback") {
            ...
            return AgentBixbyCommandMapping(intent: intent, capability: "mediaPlayback", command: mappedAction, parameters: [], score: 28)
        }
        if action == "start", let capability = firstSupported(device, ["washerOperatingState", "dryerOperatingState", "robotCleanerMovement"]) {
            return AgentBixbyCommandMapping(intent: .start, capability: capability, command: "start", parameters: [], score: 27)
        }
        if action == "pause", let capability = firstSupported(device, ["washerOperatingState", "dryerOperatingState", "robotCleanerMovement"]) {
            return AgentBixbyCommandMapping(intent: .pause, capability: capability, command: "pause", parameters: [], score: 27)
        }
        if action == "stop" || action == "cancel",
           let capability = firstSupported(device, ["washerOperatingState", "dryerOperatingState", "robotCleanerMovement"]) {
            return AgentBixbyCommandMapping(intent: .stop, capability: capability, command: "stop", parameters: [], score: 27)
        }
        ...
        if action == "show", let capability = firstSupported(device, ["videoStream", "imageCapture", "switch"]) {
            return AgentBixbyCommandMapping(intent: .getStatus, capability: capability, command: "getStatus", parameters: [], score: 25)
        }

        return nil
    }

    private func powerMapping(device: HomeCandidateRecord, command: String, intent: HomeAutomationIntent) -> AgentBixbyCommandMapping? {
        guard device.capabilities.contains("switch") else { return nil }
        return AgentBixbyCommandMapping(intent: intent, capability: "switch", command: command, parameters: [], score: 30)
    }
    ...
    private func numericMapping(
        ...
        guard let value = AgentTextParser.extractedNumbers(from: text.agentNormalizedHomeTokenString).first else {
            return nil
        }
        return AgentBixbyCommandMapping(
            intent: intent,
    ...
    private func enumMapping(text: String, capability: String, command: String, fallbackScore: Int) -> AgentBixbyCommandMapping? {
        let normalized = text.agentNormalizedHomeTokenString
        let allowed = HomeCapabilityRegistry.definitions[capability]?.enumValues ?? []
        let mode = allowed.first { normalized.contains($0.agentNormalizedHomeTokenString) } ??
            AgentTextParser.modeCandidates(in: normalized).first
        ...
        return AgentBixbyCommandMapping(
            ...
            capability: capability,
            command: command,
            parameters: [HomeResolvedParameter(name: "value", value: mode, confidence: 0.88)],
            score: fallbackScore + 1
        )
    }

    private func statusCapability(for sourceCapability: String, device: HomeCandidateRecord) -> String? {
    ...
    private func firstSupported(_ device: HomeCandidateRecord, _ capabilities: [String]) -> String? {
    ...
    private func deviceNames(for device: HomeCandidateRecord) -> [String] {
        ...
        if let aliases = device.metadata["aliases"] {
            names.append(contentsOf: aliases.split(separator: ",").map(String.init))
        }
    ...
    private func scoreMatch(
        ...
        let normalized = text.agentNormalizedHomeTokenString
        var score = 18
        if normalized.contains(deviceName.agentNormalizedHomeTokenString) {
            score += 10
        }
...
private struct AgentBixbyCommandMapping {
...
public struct BixbyFallbackAgent: HomeAgent {
    public typealias Input = BixbyFallbackInput
    public typealias Output = [AgentBixbyDraftMatch]
    ...
    public func run(_ input: BixbyFallbackInput, context: ResolutionContext) async throws -> [AgentBixbyDraftMatch] {
        try await match(input)
    }
}
