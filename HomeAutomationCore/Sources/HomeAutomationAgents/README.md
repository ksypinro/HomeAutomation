...
| Knowledge | `CapabilityKnowledgeAgent`, `BixbyKnowledgeAgent`, `CommandExampleAgent` | Retrieve relevant context, then hydrate canonical capability, Bixby, and generated-example facts. |
| Candidates | `CandidateRetrievalAgent`, `CandidateRankingAgent`, `CandidateShardAgent`, `CandidateHydrationAgent` | Find, rank, shard, and hydrate candidate devices/routines. |
...
| Component | Role |
| --- | --- |
| `CapabilityKnowledgeAgent` | Uses RAG to find relevant capability context, then hydrates canonical capability details. |
| `BixbyKnowledgeAgent` | Finds relevant Bixby commands and returns canonical Bixby snippets. |
| `CommandExampleAgent` | Retrieves generated examples similar to the user command. |
| `AgentRAGSupport` | Internal helper for retrieval calls and chunk-to-snippet formatting. |

Knowledge agents reduce prompt size and improve focus, but they do not own source-of-truth capability or risk data.
...
| `HomeCandidateResolverSupport` | Candidate scoring, ranking, sharding, and aggregation support. |
| `CandidateRetrievalAgent` | Retrieves candidates from `MockHomeDeviceRegistry`, optionally merging semantic RAG device matches. |
| `CandidateRankingAgent` | Selects final candidate IDs or asks for clarification. |
...
| `AgentTextParser` | Multilingual deterministic parser used by fallback and worker fallback logic. |
