...
`HomeAutomationOrchestrator` is the runtime coordinator for the multi-agent architecture. It plans agent work, executes sequential and parallel stages, merges context patches, streams pipeline events, records metrics, applies conversation memory, and enforces fail-closed safety behavior.
...
flowchart TD
    ...
    I --> J{"Execution policy allows mock execution?"}
    J -->|Yes| K["Run MockExecutionAgent"]
    J -->|No| L["Keep ready/confirmation/clarification/unsupported result"]
    K --> M["Assemble HomeAutomationResolverResult"]
    L --> M
...
-> MockExecutionAgent when allowed by policy
```
...
| `ResolutionContextStore` | Actor that owns the evolving `ResolutionContext`; supports snapshots, patch application, error append, trace append, and memory hint append. |
