...
public actor DailyTextLogWriter: TelemetrySink {
    ...
    private static func render(_ event: ObservabilityEvent) throws -> String {
        ...
        let tags = [
            ...
            tag("AGENT_SESSION", shortRunID(event.agentSessionID)),
            tag("AGENT_RUN", event.agentRunID.map(String.init)),
            tag("TOOL", event.toolID),
            tag("TOOL_SESSION", shortRunID(event.toolSessionID)),
            tag("TOOL_CALL", shortRunID(event.toolCallID)),
