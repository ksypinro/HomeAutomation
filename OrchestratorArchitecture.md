...
> **Status**: Production (FM-First Migration Complete)
...
graph TB
    ...
    subgraph Routing["Operation Routing"]
        OD["OperationDetectionWorkerSession"]
...
| `GraphScheduler` | `struct` | 625 | DAG-based agent execution engine |
...
| `AgentScheduler` | `struct` | 257 | Legacy phase-based execution engine |
| `AgentPlanner` | `struct` | 92 | Legacy phase-based plan construction |
...
| `AutomationRuntimeAgents` | — | 666 | Automation-specific agent implementations |
...
| `AutomationCreationResolver` | `struct` | 662 | End-to-end automation creation (legacy mode) |
...
sequenceDiagram
    ...
    participant OD as OperationDetector
    ...
    HCO->>OD: detectOperation(text)
    OD-->>HCO: automationCreation (confidence: 0.92)
...
```mermaid
graph LR
    OD["OperationDetection"] --> AD["AutomationDraft"]
    AD --> ACOR["ConditionOperandResolution"]
...
Key design: `ConditionOperandResolution` and `ActionResolution` fan out in parallel after draft extraction, then converge at the validation gate.
...
### 6.4 Dual Runtime Modes

```mermaid
graph LR
    subgraph Legacy["Legacy Mode (.legacy)"]
        AP["AgentPlanner"] --> AEP["AgentExecutionPlan"]
        AEP --> AS["AgentScheduler"]
    end

    subgraph Graph["Graph Mode (.graph) — DEFAULT"]
        GP2["GraphPlanner"] --> OG["OrchestrationGraph"]
        OG --> GV2["GraphValidator"]
        GV2 --> GS2["GraphScheduler"]
    end

    ENV["HOME_AUTOMATION_ORCHESTRATOR_RUNTIME"] -->|"graph"| Graph
    ENV -->|"legacy"| Legacy
```

Legacy mode uses flat `[AgentPhase]` arrays. Graph mode uses a full DAG with dependency tracking, guard conditions, and per-node metrics. Both coexist for rollback safety.
...
| **O-2** | **No agent-level timeout enforcement** | `AgentScheduler.runAgent()` L140, `GraphScheduler.runNode()` L252 | Every agent declares `timeoutNanoseconds` but it is **never enforced**. A hung FM call blocks the entire pipeline indefinitely. | Wrap `agent.run(context:)` in `withThrowingTaskGroup` + `Task.sleep(nanoseconds:)` race, treating timeout as a retryable failure. |
...
| **O-6** | **Legacy runtime mode is untested dead code** | `AgentScheduler`, `AgentPlanner` | The legacy scheduler is maintained alongside the graph scheduler but provides no unique functionality. Both codepaths must be kept in sync. | Deprecate and remove `AgentScheduler`/`AgentPlanner` after confirming graph mode stability in production. Gate removal behind a feature flag with a sunset date. |
...
quadrantChart
    ...
    O-6 Remove legacy runtime: [0.60, 0.70]
...
6. **O-6** — Deprecate legacy runtime (reduce surface area)
...
graph TB
    ...
    EB["AgentEventBus.swift"]
    CM["ConversationMemory.swift"]
    MC["OrchestratorMetricsCollector.swift"]
    ACR["AutomationCreationResolver.swift"]
    AAR["AutomationActionResolver.swift"]
    OG["OrchestrationGraph.swift"]
    GRM["GraphRunMetrics.swift"]
    PCK["ResolutionContextPatchKey.swift"]

    HCO --> CAA & ARA & GS & GP & AS & AP & AReg & PE & RCS & CBR & EB & CM & MC & ACR & OG & GRM
    GS --> OG & AReg & RCS & EB & PE & CBR & GV & GRM
    GP --> OGC & OG
    ACR --> AAR & CAA
    AAR --> AS & GS & AReg & PE & GP & CBR
    CAA --> AReg & PE & RCS
    ARA --> ACR & AAR
```
