...
- [x] H6: Automation repair specialists (finding 7, automation part) — `TriggerDraft` now retains the raw trigger fragment (`rawText`) and a structured, compilable device-trigger condition (`deviceCondition`); `StructuralDraftBuilder.automationTrigger` compiles device triggers, so H2's device-trigger gap is closed. `RepairSpecialistRegistry` wires `.trigger` (→ `AutomationTriggerResolutionWorkerSession` + device-condition resolution), `.conditionClause` (→ single/`BatchedConditionClauseResolver` for ≥2 disputed leaves via `RepairResult.conditionClauses`), and `.segmentation` (→ `AutomationComponentSegmentationWorkerSession`, making the shared-verb mis-split repairable); `.holisticDraft` stays intentionally unsupported. Wired in `makeVerifierLoopOrchestrator`. See [GapClosureImplementationPlan.md](GapClosureImplementationPlan.md). Tests: `AutomationRepairSpecialists` (7) + device-trigger pipeline/builder/parity tests.
